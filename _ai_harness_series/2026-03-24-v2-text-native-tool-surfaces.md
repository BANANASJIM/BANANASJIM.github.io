...
3.  **Composability:** Just like in the Unix shell, the agent can chain tools together. The output of one tool can be piped directly to the input of another: `userctl list --all | grep 'status=active' | userctl-format --template '{% raw %}{{.Name}} <{{.Email}}>{% endraw %}'`. This allows the agent to construct complex workflows from simple, modular parts without needing a new, specialized tool for every task.
